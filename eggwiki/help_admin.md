...
An Egg Wikis logo is the default for both.
...
Per default An Egg Wiki requires users to add commit messages when updating a
...
An Egg Wiki stores pages in files with names of all lowercase names. To retain
...
To enable An Egg Wiki to send mails to users registering, resetting their lost
