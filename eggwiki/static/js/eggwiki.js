...
var eggwiki_editor = {
	...
	_getSelectedLines: function () {
		const [s, e] = [
			cm_editor.getCursor('start').line,
			cm_editor.getCursor('end').line,
		]
		return [...Array(e - s + 1).keys()].map((i) => i + s)
	},
	_setLine: function (n, text) {
		cm_editor.replaceRange(text, { line: n, ch: 0 }, { line: n, ch: 999999 })
	},
	...
	_findBlock: function (selectBlock = false) {
		...
		if (selectBlock) {
			...
			cm_editor.doc.setSelection(
				{ line: block_start, ch: 0 },
				{ line: block_end, ch: block_end_len }
			)
		}
	...
	_findTable: function () {
		...
		var relative_cursor = {
			line: orig_cursor.line - block_start.line,
			ch: orig_cursor.ch,
		}
		// get selected block
		var text = cm_editor.getSelection()
		...
		if (relative_cursor.line == 0)
			relative_cursor.col = eggwiki_editor._findCursorCell(
				header,
				relative_cursor.ch
			)
		if (relative_cursor.line == 1)
			relative_cursor.col = eggwiki_editor._findCursorCell(
				align,
				relative_cursor.ch
			)
		...
		for (i in celltext) {
			...
			// calc column for relative_cursor
			if (relative_cursor.line - 2 == i)
				relative_cursor.col =
					eggwiki_editor._findCursorCell(line, relative_cursor.ch) - 1
		...
		for (j in header)
			colum_width[j] = Math.max(colum_width[j], header[j].length)
		// .. align
		...
		for (i in cells) {
			...
			for (j in cells[i])
				colum_width[j] = Math.max(colum_width[j], cells[i][j].length)
		}
		return {
			header: header,
			align: align,
			cells: cells,
			colum_width: colum_width,
	...
	_tableArray: function (t) {
		...
		for (var j in t.header) {
			row.push(
				' ' +
					eggwiki_editor._alignStr(t.header[j], t.colum_width[j], t.align[j]) +
					' '
			)
		}
		arr.push(row)
		...
		for (var j in t.align) {
			if (t.align[j] == 'left')
				row.push(':' + '-'.repeat(t.colum_width[j]) + ' ')
			else if (t.align[j] == 'right')
				row.push(' ' + '-'.repeat(t.colum_width[j]) + ':')
			else if (t.align[j] == 'center')
				row.push(':' + '-'.repeat(t.colum_width[j]) + ':')
	...
	table_add_row: function () {
		...
		for (var j in t.colum_width) {
			empty_row.push(
				eggwiki_editor._alignStr('', t.colum_width[j] + 2, t.align[j])
			)
		}
		// insert row into arr
	...
	table_remove_column: function () {
		...
		// update editor
		eggwiki_editor.table_arr_replace(
			arr,
			t.row,
			Math.min(arr[0].length - 1, t.col)
		)
		cm_editor.focus()
	},
